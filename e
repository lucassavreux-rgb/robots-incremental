...
<h1>💰 Pièces : <span id="coins">0</span></h1>
<div id="cpsDisp">Pièces/sec : <span id="cps">0</span></div>
<button id="clickBtn">Clic +1</button>

<div class="row">
  <div class="card"><h3>Générateurs</h3><div id="gens"></div></div>
  <div class="card"><h3>Upgrades</h3><div id="ups"></div></div>
  <div class="card">
    <h3>Prestige</h3>
    <button id="prestigeBtn" disabled>Reset pour <span id="prestigeGain">0</span> ⭐</button>
    <div>⭐ Prestige : <span id="stars">0</span> (×<span id="bonus">1</span> production)</div>
  </div>
</div>

<!-- TRADE -->
<div class="card"><h3>Marché du Fer</h3>
  <div>Fer : <span id="iron">0</span> – Cours : <span id="ironPrice">10</span> 💰</div>
...
let state = {
  ...
  upgrades: [
    {id:"click2", name:"Clic ×2", price:200, effect:2, bought:false, type:"click"},
    {id:"gen2",   name:"Tous les gen ×2", price:5e3, effect:2, bought:false, type:"gen"}
  ]
};
...
function prestige(){
  ...
  if(gain < 1) return;
  state.stars += gain; resetCore(); save(); log(`Prestige +${gain} ⭐`);
}
function resetCore(){
